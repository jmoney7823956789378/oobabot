...
class DecideToRespond:
    ...
    def provide_unsolicited_reply(
        self, our_user_id: int, message: typing.Union[types.ChannelMessage, types.GroupMessage]
    ) -> bool:
        """
        ...
        if hasattr(message, 'mentions') and message.mentions and not message.is_mentioned(our_user_id):
            return False
    ...
    def should_reply_to_message(
       ...
       if self.is_directly_mentioned(our_user_id, message):
         ...
         # Check if the message is in a group DM or a channel
       if isinstance(message, (types.GroupMessage, types.ChannelMessage)):
           if self.provide_unsolicited_reply(our_user_id, message):
              return (True, False)
